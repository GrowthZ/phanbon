...
import { Star, PlayCircle, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export function SocialProof() {
  ...
  const handlePrev = (e?: React.MouseEvent) => {
    ...
    setActiveIdx((prev) => (prev === 0 ? reviewImages.length - 1 : prev! - 1));
  };

  const handleNext = (e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    if (activeIdx === null) return;
    setActiveIdx((prev) => (prev === reviewImages.length - 1 ? 0 : prev! + 1));
  };
